...
function Page() {
    ...
    const { data } = useContext(AuthContext); // Access user and loading state
    useEffect(() => {
        const dataSet = data;
        try {
            if (dataSet) {
                setMessage(dataSet.Pavement_Distress);
                setsurvey(dataSet.Survey);
                setStreetLight(dataSet.Street_Light);
                setSignage(dataSet.Road_Furniture);
                setRF_chainage(dataSet.RF_chainage || []);
                setLHSChainage(dataSet.LHS_chainage || []);
                setRHSChainage(dataSet.RHS_chainage || []);
                setSelectedOption(Object.keys(dataSet.Pavement_Distress["one"][mapSide])[0]);
                setIsMounted(true);
            }
        }
        catch (error) {
    ...
    return (
        <div className="sm:text-sm ">
            <div className="p-10 mx-16 mt-14 font-bold text-black border-4 rounded-sm">
                ...
                {message && (
                    <>
                        <div className="flex justify-between w-full">
                            <div className="optionSelection   text-black py-4">
                                <select
                                    onChange={handleOptionChange}
                                    value={selectedOption}
                                    className="p-2  border border-black text-black font-bold rounded-md"
                                >
            ...
            <div className="mx-16  pt-10 border-4 rounded-sm mt-10">
                <center className="text-3xl font-bold text-black">Road Furniture</center>
                ...
                {RF === "Street_Light" ? (<div className="flex w-full gap-6 px-10 ">
                    <div className="flex-1">
                        <Street_Light
                            array={StreetLight}
                ...
                </div>) : (<div className="flex w-full gap-6 px-10  ">
                    <div className="flex-1">
                        <SignageC
                            array={Signage}
                            Lowerlimit_RF_LHS={Lowerlimit_RF_LHS}
